...
import { MAASY_ICON } from "./icon.js";
...
server.tool(
    ...
    "Auto-generates quick action pills after creation. maasy loads relevant skills based on which tool the user is using.",
  {
    ...
    slug: z
      ...
      .max(60)
      .regex(/^[a-z0-9-]+$/)
      .describe("URL-safe slug (e.g. 'meta-ads-scaling')"),
    category: z
      .enum([
        "copilot",
        "ads",
    ...
    content: z
      ...
      .min(50)
      .describe(
        "Knowledge content in markdown. Include frameworks, processes, best practices, examples. Max ~2000 words."
      ),
...
server.resource("active-brand", "maasy://brand/active", async (uri) => {
      ...
      contents: [
        {
          uri: uri.href,
          mimeType: "text/plain",
          text: "No default brand. Set MAASY_PROJECT_ID or use maasy_list_brands.",
        },
      ],
    };
  }
...
server.resource("skills-catalog", "maasy://skills/catalog", async (uri) => {
  try {
